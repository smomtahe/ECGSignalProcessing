...
for i = 1:2
    ...
    %%Reduction of EMG noise %calculate the empirical cumulative distribution function (ECDF) of the 4th level detail coefficient (cd4).
    %cd4
    elem4 = sum(l(1:7) + 1);
    elem41 = sum(l(1:8));
...
[locs pks] = Rpeakfinder(final_signal(:, 1), 180, 0.11); % 1: 1st channel % 180 fs (number of samples collected per second) %0.11:threshold for peak detection. Peaks with amplitudes lower than this threshold may not be identified as R-peaks.
